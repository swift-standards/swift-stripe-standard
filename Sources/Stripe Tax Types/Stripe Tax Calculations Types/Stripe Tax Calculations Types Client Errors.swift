//
//  Stripe Tax Calculations Types Client Errors.swift
...
extension Stripe.Tax.Calculations.Create {
    /// Failure leaf for `Stripe.Tax.Calculations.Client.create`.
    ///
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered, but the payload did not decode as `Stripe.Tax.Calculation`.
...
extension Stripe.Tax.Calculations.Retrieve {
    /// Failure leaf for `Stripe.Tax.Calculations.Client.retrieve`.
    ///
    /// Domain leaf of the ruled `Either<Async.Lifecycle.Error, Leaf>` witness-error
    /// shape (swift-foundations/swift-witnesses#3, ratified by comment 5143970225).
    /// Lifecycle facts — shutdown and timeout among them — ride in
...
extension Stripe.Tax.Calculations.ListLineItems {
    /// Failure leaf for `Stripe.Tax.Calculations.Client.listLineItems`.
    ///
    /// Domain leaf of the ruled `Either<Async.Lifecycle.Error, Leaf>` witness-error
    /// shape (swift-foundations/swift-witnesses#3, ratified by comment 5143970225).
    /// Lifecycle facts — shutdown and timeout among them — ride in
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered with a structured API error.
        case api(StripeError.Error)
        /// Stripe answered, but the payload did not decode as `List.LineItems.Response } }`.
        case malformedResponse
        /// The witness carries no implementation for this operation.
        case notImplemented(Witness.Unimplemented.Error)
