//
//  Stripe Tokens Types Client Errors.swift
...
extension Stripe.Tokens.Create {
    /// Failure leaf for `Stripe.Tokens.Client.create`.
    ///
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered, but the payload did not decode as `Stripe.Tokens.Token`.
...
extension Stripe.Tokens.Retrieve {
    /// Failure leaf for `Stripe.Tokens.Client.retrieve`.
    ///
    /// Domain leaf of the ruled `Either<Async.Lifecycle.Error, Leaf>` witness-error
    /// shape (swift-compositions/swift-witnesses#3, ratified by comment 5143970225).
    /// Lifecycle facts — shutdown and timeout among them — ride in
