//
//  Stripe Balance Transactions Types Client Errors.swift
...
extension Stripe.BalanceTransactions.Retrieve {
    /// Failure leaf for `Stripe.BalanceTransactions.Client.retrieve`.
    ///
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered, but the payload did not decode as `Stripe.Balance.Transaction`.
...
extension Stripe.BalanceTransactions.List {
    /// Failure leaf for `Stripe.BalanceTransactions.Client.list`.
    ///
    /// Domain leaf of the ruled `Either<Async.Lifecycle.Error, Leaf>` witness-error
    /// shape (swift-compositions/swift-witnesses#3, ratified by comment 5143970225).
    /// Lifecycle facts — shutdown and timeout among them — ride in
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered with a structured API error.
        case api(StripeError.Error)
        /// Stripe answered, but the payload did not decode as `Stripe.BalanceTransactions.List.Response } }`.
        case malformedResponse
        /// The witness carries no implementation for this operation.
        case notImplemented(Witness.Unimplemented.Error)
