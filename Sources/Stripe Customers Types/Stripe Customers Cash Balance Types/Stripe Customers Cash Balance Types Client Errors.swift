//
//  Stripe Customers Cash Balance Types Client Errors.swift
...
extension Stripe.Customers.CashBalance.Retrieve {
    /// Failure leaf for `Stripe.Customers.CashBalance.Client.retrieve`.
    ///
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered, but the payload did not decode as `Stripe_Types_Models.CashBalance`.
...
extension Stripe.Customers.CashBalance.Update {
    /// Failure leaf for `Stripe.Customers.CashBalance.Client.update`.
    ///
    /// Domain leaf of the ruled `Either<Async.Lifecycle.Error, Leaf>` witness-error
    /// shape (swift-foundations/swift-witnesses#3, ratified by comment 5143970225).
    /// Lifecycle facts — shutdown and timeout among them — ride in
