//
//  Stripe Customers Cash Balance Transactions Types Client Errors.swift
...
extension Stripe.Customers.CashBalanceTransactions.Retrieve {
    /// Failure leaf for `Stripe.Customers.CashBalanceTransactions.Client.retrieve`.
    ///
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered, but the payload did not decode as `CashBalanceTransaction`.
...
extension Stripe.Customers.CashBalanceTransactions.List {
    /// Failure leaf for `Stripe.Customers.CashBalanceTransactions.Client.list`.
    ///
    /// Domain leaf of the ruled `Either<Async.Lifecycle.Error, Leaf>` witness-error
    /// shape (swift-compositions/swift-witnesses#3, ratified by comment 5143970225).
    /// Lifecycle facts — shutdown and timeout among them — ride in
