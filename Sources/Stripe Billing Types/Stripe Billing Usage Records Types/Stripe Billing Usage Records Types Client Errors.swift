//
//  Stripe Billing Usage Records Types Client Errors.swift
...
extension Stripe.Billing.UsageRecords.Create {
    /// Failure leaf for `Stripe.Billing.UsageRecords.Client.create`.
    ///
        ...
        Witness.Unimplemented.Representable
    {
        ...
        /// Stripe answered, but the payload did not decode as `UsageRecord`.
...
extension Stripe.Billing.UsageRecords.List {
    /// Failure leaf for `Stripe.Billing.UsageRecords.Client.list`.
    ///
    /// Domain leaf of the ruled `Either<Async.Lifecycle.Error, Leaf>` witness-error
    /// shape (swift-compositions/swift-witnesses#3, ratified by comment 5143970225).
    /// Lifecycle facts — shutdown and timeout among them — ride in
